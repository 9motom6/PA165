...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductByName(){
		List<Product> found = em.createQuery("SELECT p FROM Product p WHERE name = :name",Product.class)
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em
                .createQuery("SELECT NEW cz.fi.muni.pa165.tasks.ProductAndCategory(p,c) FROM Product p LEFT JOIN p.categories c ON c.name = 'Kitchen'", ProductAndCategory.class)
                .getResultList();
